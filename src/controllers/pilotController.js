...
try {
    ...
    explanation = 'This command resets your branch to the state it was in before the merge.';
    } else if (firstLine.includes('rebase')) {
    command = 'git rebase --abort';
    explanation = 'This command completely cancels the current rebase operation.';
    } else if (firstLine.includes('commit')) {
    command = 'git reset --soft HEAD~1';
    explanation = 'This command undoes your last commit but keeps all your changes staged.';
    } else {
    return res.json({ command: null, explanation: "Couldn't determine a safe action to undo." });
    }
